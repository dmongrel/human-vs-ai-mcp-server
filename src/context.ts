...
This module is intentionally extensible: each detector is an isolated file implementing the Detector interface, registered in src/lib/detectors/index.ts. Adding a new built-in signal means adding one file and one array entry — no changes needed elsewhere. (Two model-perplexity signals exist in the code but are currently disabled: a model-runner-based one, disabled after investigation found the technique unreliable, and a bundled llama.cpp-engine one that does proper teacher-forced scoring but whose score calibration is still provisional — see README.md's "Model runner (currently disabled)" and "Bundled engine perplexity (currently disabled)" sections.) Third parties can also add signals without forking the project at all, by dropping plugin files in a directory pointed to by the PLUGINS_DIR environment variable — see README.md's "Plugins" section.
