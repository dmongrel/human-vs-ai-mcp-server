...
This module is intentionally extensible: each detector is an isolated file implementing the Detector interface, registered in src/lib/detectors/index.ts. Adding a new built-in signal means adding one file and one array entry — no changes needed elsewhere. (There are two model-perplexity signals. The bundled llama.cpp-engine one does proper teacher-forced scoring and is enabled, though it only contributes when LLAMA_ENGINE_MODEL_PATH points at a .gguf model, and its thresholds are calibrated against one specific model on a small sample — treat it as corroborating evidence, not a verdict. The older model-runner-based one (HTTP, via MODEL_RUNNER_URL) is also enabled but opt-in, and makes no network call unless that variable is set. It is verified to work, but its technique reads logprobs off the model's own temperature-0 reproduction of the text, which is near-certain by construction — its perplexity collapses toward 1.0 largely regardless of input. If it appears in a report, describe it as diagnostic and do not cite it as evidence of authorship; the bundled-engine number is the one to reason from. The paragraph-coherence signal is also disabled: measurement showed its premise (AI staying more tightly on-topic) does not hold for narrative prose, so it discriminated in neither direction. See README.md's "Bundled engine perplexity" and "Model runner (HTTP, opt-in)" sections.) Third parties can also add signals without forking the project at all, by dropping plugin files in a directory pointed to by the PLUGINS_DIR environment variable — see README.md's "Plugins" section.
