...
test("model-perplexity recommendation stays disabled even when the runner is configured and would succeed", async () => {
  ...
  // The HTTP model-runner signal is disabled via `enabled: false` on the
  // detector itself (src/lib/detectors/modelPerplexity.ts), so the
  // orchestrator filters it out before run() and no request is ever made.
