...
| `weights` | `Record<string, number>` | no | Override specific detector weights for this call only, keyed by detector id (e.g. `{ "readability-uniformity": 0.05 }`). Applied after `type` selects the ruleset defaults. An id matching no active detector is ignored, not rejected. A negative weight throws. Never changes the server's actual defaults — scoped to the single call. Call `get_context({ topic: "detect_ai_usage" })` for the full id/weight table, generated live from each detector's own `weight()` function so it can't drift from the hardcoded per-ruleset defaults in `src/lib/detectors/*.ts`. |
...
**Input**: same shape as `detect_ai_usage` (`text` / `filePath`, plus optional `reportPath`, `type`, `ignoreMd`, and `weights`). If you called `detect_ai_usage` with a `type` (and/or `ignoreMd`, `weights`), pass the same values to `humanize_text` for consistent recommendations — `weights` is forwarded straight through to the internal `detectAiUsage` call, so it affects `aiLikelihoodScore` the same way it would there.
...
- [x] Optional model-runner-perplexity detector (real perplexity check against a local model runner) implemented, investigated, and **disabled** — mechanically works (verbatim-echo via `/v1/chat/completions`, verified end-to-end on live LM Studio and Ollama instances across several models) but structurally biased toward ~1.0 for any successfully-reproduced text regardless of content, plus slow on every runner tested. Briefly enabled and re-measured, which confirmed the finding rather than overturning it. Not to be confused with the bundled engine, which is the perplexity signal this project uses. See README's "HTTP model runner (disabled)" section for the full investigation, `src/lib/modelRunner.ts`, and `enabled` in `src/lib/detectors/modelPerplexity.ts`.
...
- [x] Published to npm as [`human-vs-ai-mcp-server`](https://www.npmjs.com/package/human-vs-ai-mcp-server) (see [CHANGELOG.md](./CHANGELOG.md) for the current version), alongside the prebuilt engine package `human-vs-ai-mcp-server-win32-x64`, which installs automatically on Windows x64. Source at [dmongrel/human-vs-ai-mcp-server](https://github.com/dmongrel/human-vs-ai-mcp-server).
- [x] Per-call `weights` override on both `detect_ai_usage` and `humanize_text`: any detector's weight can be overridden for a single call, keyed by detector id, without editing source or rebuilding. `get_context({ topic: "detect_ai_usage" })` reports the live id/weight table, generated from each detector's own `weight()` function rather than hand-copied, so it always matches the hardcoded per-ruleset defaults in `src/lib/detectors/*.ts`. See `listDetectorWeights`/`formatDetectorWeightsTable` in `src/lib/detectAiUsage.ts`.
