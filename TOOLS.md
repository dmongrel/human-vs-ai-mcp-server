...
**Signals** (see `src/lib/detectAiUsage.ts`): sentence-length burstiness, lexical diversity (rolling type-token ratio), AI stock-phrase frequency, readability uniformity across paragraphs, markdown-in-prose artifacts, em dash overuse. A 7th signal — a real perplexity check against a local model via `MODEL_RUNNER_URL` — is implemented but **currently disabled** (`MODEL_PERPLEXITY_SIGNAL_ENABLED = false`); see [Model runner (currently disabled)](./README.md#model-runner-currently-disabled) in the README for why. Each is an isolated function — add a new signal by adding one function and one entry in the `detectors` array. Note: `detectAiUsage` is `async` (it awaits the optional model-runner call, even though that call is currently a no-op); `humanize_text` and the tool handlers in `src/index.ts` await it accordingly.
